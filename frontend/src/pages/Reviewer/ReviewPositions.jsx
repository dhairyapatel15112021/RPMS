...
export const ReviewPositions = () => {
    ...
    const applicationModal = "application_modal";

    const getOpenAssignedPositions = async () => {
        try {
            const response = await axios.get(`${ApiEndPoints.getAllReviewePosition}${data.user.id}`, { headers: { Authorization: localStorage.getItem("token") } });
    ...
    const setApplicationSettings = (id) => {
        setPositionId(() => id);
        document.getElementById(applicationModal).showModal();
    }

    return (
        <div className='p-2 shadow-md mt-3 rounded-md w-full overflow-hidden'>
            <ApplicationModal empId={data.user.id} id={applicationModal} positionId={positionId} />
